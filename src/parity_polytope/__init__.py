...
fun_proj_vec = lib.proj_vec
fun_proj_vec.argtypes = [ctypes.c_size_t, ndp_dbl, ndp_dbl]


def proj_vec(arr_in, arr_out=None):
    arr_out = np.zeros_like(arr_in, dtype=arr_in.dtype) if arr_out is None else arr_out
    fun_proj_vec(arr_in.size, arr_in, arr_out)
    return arr_out
...
def proj_csr(csr_in):
    ...
    fun_proj_csr(csr_in.indptr.size, csr_in.indptr, csr_in.data, arr_out)
    return arr_out
